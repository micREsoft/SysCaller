...
void GeneralTab::initUI()
{
    ...
    QRadioButton* directAssemblyRadio = new QRadioButton("Direct Syscall Mode (Default)");
    directAssemblyRadio->setToolTip("Use traditional instruction mnemonics for syscall stubs");

    QRadioButton* inlineAssemblyRadio = new QRadioButton("Inline Syscall Mode");
    inlineAssemblyRadio->setToolTip("Generate MASM compatible db-based stubs");

    QRadioButton* indirectAssemblyRadio = new QRadioButton("Indirect Syscall Mode");
...
void GeneralTab::onModeChanged()
{
    ...
    if (isKernelMode)
    {
        ...
        if (inlineAssemblyRadio)
        {
            inlineAssemblyRadio->setEnabled(false);
        }

        if (indirectAssemblyRadio)
        {
            indirectAssemblyRadio->setEnabled(false);
        }

        if (directAssemblyRadio)
        {
            directAssemblyRadio->setEnabled(true);
        }
    }
    else
    {
        if (inlineAssemblyRadio)
        {
            inlineAssemblyRadio->setEnabled(true);
        }

        if (indirectAssemblyRadio)
        {
            indirectAssemblyRadio->setEnabled(true);
        }
