...
extern "system" {
    ...
    fn SysClose(handle: HANDLE) -> NTSTATUS;
}
...
fn InjectDLL(process_handle: HANDLE, dll_path: &str) -> bool {
    unsafe {
        ...
        let status = SysAllocateVirtualMemoryEx(
            process_handle,
            &mut base_address,
            &mut region_size,
        ...
        let status = SysWriteVirtualMemory(
            process_handle,
            base_address,
