...
from .utils import format_timestamp, get_project_paths, create_backup, get_available_backups
    ...
    def restore_backup(self, timestamp):
        try:
            ...
            if not os.path.exists(asm_dir):
                ...
                # DEBUG print(f"Created ASM directory: {asm_dir}")
            if not os.path.exists(header_dir):
                os.makedirs(header_dir, exist_ok=True)
                # DEBUG print(f"Created header directory: {header_dir}")
            if os.path.exists(asm_path):
                if is_file_locked(asm_path):
                    ...
                    try:
                        ...
                        # DEBUG print(f"Changed permissions on ASM file to make it writable")
                    except Exception as e:
            ...
            if os.path.exists(header_path):
                if is_file_locked(header_path):
                    ...
                    try:
                        ...
                        # DEBUG print(f"Changed permissions on header file to make it writable")
                    except Exception as e:
                        print(f"Failed to change permissions: {e}")
            if self.create_backup.isChecked():
                self.settings.setValue('general/create_backup', True)
                create_backup(self, self.settings)
            # DEBUG print(f"Restoring ASM file from {backup_asm_path} to {asm_path}")
            asm_restored = False
            max_retries = 3
            retry_count = 0
            while not asm_restored and retry_count < max_retries:
                try:
                    with open(backup_asm_path, 'rb') as src_file:
                        ...
                        # DEBUG print(f"Read {len(asm_content)} bytes from ASM backup file")
                    with open(asm_path, 'wb') as dst_file:
                        dst_file.write(asm_content)
                        # DEBUG print(f"Wrote {len(asm_content)} bytes to ASM destination file")
                    if os.path.exists(asm_path):
                        file_size = os.path.getsize(asm_path)
                        # DEBUG print(f"Verified ASM file exists with size: {file_size} bytes")
                        if file_size > 0:
                            asm_restored = True
                        else:
                    ...
                    if not asm_restored:
                        # DEBUG print(f"Trying alternative method for ASM file (shutil.copy2)")
                        shutil.copy2(backup_asm_path, asm_path)
                        if os.path.exists(asm_path) and os.path.getsize(asm_path) > 0:
                            # DEBUG print(f"Alternative method succeeded for ASM file")
                            asm_restored = True
                        else:
                            # DEBUG print(f"Trying system command for ASM file")
                            import subprocess
                            try:
                                ...
                                if os.path.exists(asm_path) and os.path.getsize(asm_path) > 0:
                                    # DEBUG print(f"System command succeeded for ASM file")
                                    asm_restored = True
                            except Exception as e:
                    ...
                    if retry_count < max_retries:
                        # DEBUG print(f"Retrying in 3 seconds...")
                        time.sleep(3)
                    else:
                        QMessageBox.warning(self, "Restore Error", f"Failed to restore ASM file after {max_retries} attempts: {str(e)}")
            # DEBUG print(f"Restoring Header file from {backup_header_path} to {header_path}")
            ...
            while not header_restored and retry_count < max_retries:
                try:
                    with open(backup_header_path, 'rb') as src_file:
                        ...
                        # DEBUG print(f"Read {len(header_content)} bytes from header backup file")
                    with open(header_path, 'wb') as dst_file:
                        dst_file.write(header_content)
                        # DEBUG print(f"Wrote {len(header_content)} bytes to header destination file")
                    if os.path.exists(header_path):
                        file_size = os.path.getsize(header_path)
                        # DEBUG print(f"Verified header file exists with size: {file_size} bytes")
                        if file_size > 0:
                    ...
                    if not header_restored:
                        # DEBUG print(f"Trying alternative method for header file (shutil.copy2)")
                        shutil.copy2(backup_header_path, header_path)
                        if os.path.exists(header_path) and os.path.getsize(header_path) > 0:
                            # DEBUG print(f"Alternative method succeeded for header file")
                            header_restored = True
                        else:
                            # DEBUG print(f"Trying system command for header file")
                            import subprocess
                            try:
                                ...
                                subprocess.run(cmd, shell=True, check=True)
                                if os.path.exists(header_path) and os.path.getsize(header_path) > 0:
                                    # DEBUG print(f"System command succeeded for header file")
                                    header_restored = True
                            except Exception as e:
                                print(f"System command failed: {e}")
                except Exception as e:
